...
public class StrikeDatabase extends SQLiteOpenHelper {
    ...
    public StrikeDatabase(Context ctx) { super(ctx, DB_NAME, null, DB_VERSION); }

    @Override public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + TABLE + " (" +
                "id INTEGER PRIMARY KEY AUTOINCREMENT," +
                "speed_ms REAL, power_score REAL, technique_score REAL," +
                "power_level REAL, punch_type TEXT, technique_note TEXT," +
                "timestamp_ms INTEGER, is_perfect INTEGER)");
    }
    ...
    public void saveStrike(StrikeTracker.StrikeResult r) {
        ...
        v.put("technique_note",  r.techniqueNote);
    ...
    public StrikeTracker.StrikeResult getPersonalBest(String metric) {
        String col = metric.equals("speed") ? "speed_ms"
                   : metric.equals("power") ? "power_score" : "power_level";
    ...
    public int getPerfectStrikes() {
        Cursor c = getReadableDatabase().rawQuery(
                "SELECT COUNT(*) FROM " + TABLE + " WHERE is_perfect=1", null);
        int n = 0; if (c.moveToFirst()) n = c.getInt(0); c.close(); return n;
    }
}
